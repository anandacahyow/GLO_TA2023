...
def updater_entrypoint(contexts, id, period, val_data):
    ...
    while 1:
        ...
        if t < n_past:
            """p = NN_Model_Params(model_wc1, p, 14, x[['wc11']])
            q = NN_Model_Params(model_ch1, q, 14, x[['ch11']])
            r = NN_Model_Params(model_gor1, r, 14, x[['gor11']])


            i = NN_Model_Params(model_wc2, i, 14, x2[['wc22']])
            j = NN_Model_Params(model_ch2, j, 14, x2[['ch22']])
            k = NN_Model_Params(model_gor2, k, 14, x2[['gor22']])"""
        ...
        dict_data = {113: [qt_simulated,wc_val,qt_cum_val,qo_simulated,qo_cum_val,q[-1],r[-1]],
                    114: setpoint,
                    115: [qt_simulated2,wc_val2,qt_cum_val2,qo_simulated2,qo_cum_val2,j[-1],k[-1]],
                    116: setpoint2,
                    }
